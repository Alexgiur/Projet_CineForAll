...
<header class="main-header">
    ...
    <nav class="main-nav">
        <ul>
            ...
            @endif
            @guest
                <li><a href="{{ url('/login') }}" class="cta-login">Connexion</a></li>
            @endguest
            @auth
                <li>
...
<main class="admin-dashboard">
    ...
    <div class="admin-toolbar" style="display: flex; justify-content: space-between; align-items: center;">
        <form action="{{ url('/personnes') }}" method="GET" class="search-container" style="flex-grow: 1; margin-right: 20px;">
            ...
            @if($search)
                <a href="{{ url('/personnes') }}" class="btn-edit" style="background-color: #6c757d; text-decoration: none; display: flex; align-items: center;">Réinitialiser</a>
            @endif
        </form>

        @auth
            @if(Auth::user()->IdTypeRoleUti == 1)
                <a href="{{ url('/personnes/create') }}" class="btn-edit" style="background-color: var(--green-btn); white-space: nowrap;">+ Ajouter une personne</a>
            @endif
        @endauth
    </div>
        ...
        @foreach($personnes as $personne)
            <tr>
                ...
                <td style="display: flex; gap: 10px; justify-content: center;">
                    <a href="{{ url('/personnes/' . $personne->Idper) }}" class="details-link" style="margin:0;">Voir</a>
                    @auth
                        @if(Auth::user()->IdTypeRoleUti == 1)
                            <a href="{{ url('/personnes/' . $personne->Idper . '/edit') }}" class="btn-edit">Modifier</a>
                            <form action="{{ url('/personnes/' . $personne->Idper) }}" method="POST" onsubmit="return confirm('Supprimer ?');">
                                @csrf
                                @method('DELETE')
