...
<main class="admin-dashboard" style="padding: 20px;">
    ...
    <section style="background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); margin-bottom: 40px;">
        ...
        <form action="{{ url('/admin/programmations') }}" method="POST" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; align-items: end;">
            @csrf
            <div>
            ...
            @foreach($programmations as $prog)
                <tr style="border-bottom: 1px solid #eee;">
                    ...
                    <td style="padding: 15px;">
                        <form action="{{ url('/admin/programmations/' . $prog->IdProg) }}" method="POST">
                            @csrf @method('DELETE')
