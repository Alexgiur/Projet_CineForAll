    ...
    @auth
        @if(Auth::user()->IdTypeRoleUti == 1)
            <div style="text-align: center; margin-bottom: 20px;">
                <a href="{{ url('/cinemas/create') }}" class="btn-submit" style="display: inline-block; padding: 10px 20px; text-decoration: none;">Ajouter un cinéma</a>
            </div>
        @endif
    @endauth
                ...
                <tr>
                    ...
                    <td>
                        <div class="table-actions">
                            <a href="{{ url('/cinemas/' . $cinema->IdCinema) }}" class="btn-menu-uniforme" style="padding: 6px 12px; font-size: 0.9em; background-color: var(--blue-btn);">Voir</a>
                            @auth
                                @if(Auth::user()->IdTypeRoleUti == 1)
                                    <a href="{{ url('/cinemas/' . $cinema->IdCinema . '/edit') }}" class="btn-menu-uniforme" style="padding: 6px 12px; font-size: 0.9em; background-color: var(--blue-btn);">Modifier</a>
                                    <form action="{{ url('/cinemas/' . $cinema->IdCinema) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce cinéma ?');" style="margin: 0;">
                                        @csrf
                                        @method('DELETE')
