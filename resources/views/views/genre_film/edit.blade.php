...
<head>
    ...
    <title>CineForAll - Modifier un genre</title>
...
<main>
    <div class="genre-form-container">
        ...
        <form method="POST" action="{{ url('/genre_film/' . $genre_film->IdGenreFilm) }}">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="LibGenreFilm">Libellé du genre</label>
                <input type="text" id="LibGenreFilm" name="LibGenreFilm"
                       value="{{ old('LibGenreFilm', $genre_film->LibGenreFilm) }}">
                @error('LibGenreFilm')
            ...
            <div class="form-actions">
                <a href="{{ url('/genre_film') }}" style="color: #555; text-decoration: underline;">
                    ← Annuler
                </a>
                <button type="submit" class="btn-menu-uniforme">Mettre à jour</button>
            </div>
        </form>
    </div>
</main>
