                    ...
                    @foreach($reservations as $res)
                        <tr style="border-bottom: 1px solid #ecf0f1;">
                            ...
                            <td style="padding: 15px; text-align: center; display: flex; gap: 10px; justify-content: center;">
                                ...
                                @if($isFuture)
                                    <a href="{{ url('/mes-reservations/' . $res->IdRes . '/edit') }}" class="btn-menu-uniforme" style="background-color: #3498db !important; padding: 8px 15px; font-size: 0.9em;">
                                        Modifier
                                    </a>

                                    <form action="{{ url('/mes-reservations/' . $res->IdRes) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment annuler cette réservation ?');">
                                        @csrf
                                        @method('DELETE')
