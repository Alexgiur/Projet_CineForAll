...
@section('title', 'CineForAll - Modifier ' . $personne->PrePer)

@section('content')
    <div class="create-section">
        <div class="form-container">
            <h1>Modifier le Profil</h1>
            <form action="{{ route('personnes.update', $personne->Idper) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <a href="{{ route('personnes.index') }}" style="display:block; text-align:center; margin-top:15px; color:#777;">Annuler</a>
            </form>
        </div>
    </div>
@endsection
